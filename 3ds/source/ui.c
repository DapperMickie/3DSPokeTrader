...
void ui_status_frame(const char *state,const char *detail,const char *received,const char *mode,unsigned elapsed_ms) {
    ...
    int preview=running && received && received[0];
    ...
    text(270,185,done?"RECEIVED":(offer || preview)?"SWITCH OFFER":"FROM SWITCH",0,NAVY,110);
    ...
    text(22,161,(done || offer || preview)?received:running?"Waiting for the Switch selection...":"Recovery record kept on SD card.",0,NAVY,274);
