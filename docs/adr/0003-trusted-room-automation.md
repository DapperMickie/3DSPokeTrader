...
The Switch browser control page, short-code comparison, offer buttons, and final saved button are removed from the normal flow. The 3DS displays the validated incoming offer during the active exchange. A matching receipt is released automatically when the Switch protocol commits the trade, while link cleanup may continue in the background. Missing or mismatched receipts remain uncertain and require manual recovery; radio trades are never automatically repeated after interruption.
